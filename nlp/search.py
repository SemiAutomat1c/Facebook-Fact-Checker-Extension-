...
class GoogleCSESearcher:
    ...
    def __init__(self, config_name="gmanetwork"):
        ...
        self.filepath = os.path.join(self.output_dir, self.fixed_filename)
    ...
    def _load_existing_results(self):
        ...
        if os.path.exists(self.filepath):
            try:
                ...
                return {'config': self.config_name, 'searches': []}
        return {'config': self.config_name, 'searches': []}
    
    def _save_combined_results(self, claim, analysis, search_terms, all_results):
        """Save comprehensive analysis results to the fixed JSON file"""
        # Load existing data
        ...
        new_search = {
            ...
            'claim': claim,
            'analysis': analysis,
            'search_terms_used': search_terms[:3],  # Top 3 terms used
            'total_results_found': len(all_results),
            'results': all_results[:50]  # Save top 50 results
        }
        
        # Add to searches list
        existing_data['searches'].append(new_search)
        
        # Keep only last 20 searches to prevent file from growing too large
        if len(existing_data['searches']) > 20:
            existing_data['searches'] = existing_data['searches'][-20:]
        ...
        try:
            ...
            print(f"\n💾 Results appended to: {self.filepath}")
            print(f"📊 File size: {file_size:,} bytes")
            print(f"📋 Total searches in file: {len(existing_data['searches'])}")
            return True
    ...
    def search(self, search_term, save_files=True):
        ...
        try:
            ...
            processed_results = self._extract_results(data)
            ...
            if 'results' in data:
                for item in data['results']:
                    results.append({
                        ...
                        'url': item.get('unescapedUrl', item.get('url', '')),
                        'visibleUrl': item.get('visibleUrl', ''),
                        'cacheUrl': item.get('cacheUrl', '')
                    })
            elif 'items' in data:
                for item in data['items']:
                    results.append({
                        'title': item.get('title', ''),
                        'content': item.get('snippet', item.get('htmlSnippet', '')),
                        'url': item.get('link', ''),
                        'visibleUrl': item.get('displayLink', ''),
                        'cacheUrl': item.get('cacheId', '')
                    })
    ...
    def search_with_claim(self, claim):
        ...
        print("\n" + "="*70)
        print(f"🔬 CLAIM VERIFICATION SYSTEM - {self.config_name.upper()}")
        print("="*70)
        
        # Step 1: Analyze the claim
        print("\n📊 STEP 1: Analyzing claim...")
        analysis = self.claim_optimizer.analyze_claim(claim)
        
        # Step 2: Generate search terms
        print("\n🎯 STEP 2: Generating optimized search terms...")
        search_terms, weights = self.claim_optimizer.generate_search_terms(analysis)
        ...
        # Save combined results to the fixed JSON file
...
def main():
    ...
    print(f"   - ScrapedNews/Scraped/gma.json")
    print(f"   - ScrapedNews/Scraped/inquirer.json")
    print("=" * 70)
    ...
    for config in configs:
        ...
        searcher.search_with_claim(claim)
